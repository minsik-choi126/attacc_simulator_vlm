...
* python 3.8.8 (also verified on 3.10.12)
...
`pandas` is required for `dgx-attacc` (PIM/Ramulator cache); `numpy` is pulled in
as a pandas dependency. GPU-only paths (`dgx`, `dgx-cpu`) work without pandas
but the import is gated, so installing it up-front avoids surprises.
...
Note: `set_pim_ramulator.sh` resets the `ramulator2` submodule to commit
`b7c70275f04126c647edb989270cc429776955d1`. Verify that this commit is
reachable in the upstream you cloned from before applying patches.
...
$ python main.py --help
    ...
    parser.add_argument("--gpu", type=str, default='A100a',
            help="GPU type (A100a, A100, H100, A6000); A100a is A100 with HBM3, A6000 is RTX A6000 (Ampere, GDDR6 48 GB)")
    parser.add_argument("--ngpu", type=int, default=8,
            help="number of GPUs")
    parser.add_argument("--gmemcap",
                        type=int,
                        default=None,
                        help="memory capacity per GPU (GB). default=per-GPU spec (A100a=80, H100=80, A6000=48); set to override")
    parser.add_argument("--interface", type=str, default='NVLINK3',
            help="inter-GPU/AttAcc link: NVLINK3 / NVLINK4 / NVLINK_BRIDGE (A6000) / PCIE4 / PCIE5")
